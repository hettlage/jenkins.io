...
Add a path node in the file "/usr/local/Cellar/jenkins/NNN/homebrew.mxcl.jenkins-lts.plist" (where NNN is replaced withe the version number) like this:

.Contents of homebrew.mxcl.jenkins.plist
[source,xml]
----
...
Now restart jenkins using "brew services restart jenkins" 
